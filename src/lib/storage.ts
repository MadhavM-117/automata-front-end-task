...
export const isValidTurn = (value: string): value is TurnOption => {
	return ["scissors", "paper", "rock", "lizard", "spock"].includes(value as TurnOption)
}
...
export const extractValidTurnHistory = (savedHistory: string | null): TurnOption[] => {
...
export const getGameState = (): InternalGameState => {
	...
	return {
		...
		playerTurnHistory: extractValidTurnHistory(localStorage.getItem('automata-turn-history'))
	}
}

export const setGameState = ({ username, currentScore, playerTurnHistory }: InternalGameState): void => {
	if (username !== null) {
		localStorage.setItem('automata-username', username);
	}

	localStorage.setItem('automata-score', currentScore.toString());
	localStorage.setItem('automata-turn-history', JSON.stringify(playerTurnHistory));
}
